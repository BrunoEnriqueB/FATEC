...
void ocorrencias(char *p) // recebe o ponteiro da palavra
{
  ...
  while (*p != '\0') // while que vai rodar a palavra
  {
    char letra = *p; // pega a letra que está sendo verificada na hora

    for (char *i = p; *i != '\0'; i++) // for que vai rodar da letra atualmente indexada até o final da palavra
    {
      if (i == p && *i != '-') // verifica se a letra atual é a primeira que aparece
      {
        vetorOcorrencias[cont]++; // se sim, adiciona a primeira ocorrencia
      }
      else if (*i == letra && *i != '-') // verifica se existem outros caracteres iguais a variável letra, sendo diferente de '-'
      {
    ...
    if (vetorOcorrencias[cont] != 0) // só vai pro próximo index do vetorOcorrencias se houve alguma ocorrencia da letra sendo verificada
    {
      cont++;
    }
  ...
  p = inicio; // volta p para o endereço inicial
  cont = 0;
  while (*p != '\0') // printa na tela as ocorrencias das letras na palavra
  {
    if (*p != '-')
    {
